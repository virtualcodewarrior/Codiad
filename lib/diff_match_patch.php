	...
	function diff_linesToCharsMunge($text, &$lineArray, &$lineHash) {
		...
		while ($lineEnd < mb_strlen($text) - 1) {
			...
			if ( isset($lineHash[$line]) ) {
				$chars .= mb_char($lineHash[$line]);
			} else {
				$chars .= mb_char($lineArrayLength);
...
function charCodeAt($str, $pos) {
	return mb_ordinal(mb_substr($str, $pos, 1));
}
function mb_ordinal($v) {
	$k = mb_convert_encoding($v, 'UCS-2LE', 'UTF-8'); 
	$k1 = ord(substr($k, 0, 1)); 
	$k2 = ord(substr($k, 1, 1)); 
	return $k2 * 256 + $k1; 
}
function mb_char($num){
